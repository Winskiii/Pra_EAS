<template>
    <div>
      ...
      <form @submit.prevent="handleSignUp">
        ...
        <input v-model="fullName" id="fullName" name="fullName" type="text" />
  ...
  export default {
    setup() {
      ...
      const handleSignUp = async () => {
        try {
          const resp = await fetch("http://localhost:3200/api/customers", {
            method: "POST",
            credentials: "include",
            headers: {
      ...
      return { email, password, fullName, birthday, handleSignUp, goBack };
    },
  };
  </script>
